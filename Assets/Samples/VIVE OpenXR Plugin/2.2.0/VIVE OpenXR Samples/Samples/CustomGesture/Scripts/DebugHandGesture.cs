...
public class DebugHandGesture : MonoBehaviour
{
    ...
    public CustomGestureManager HGM;
    public CustomGestureDefiner GD;

    public Text[] startRecord;
    public bool stateRecL;
    public bool stateRecR;
    public bool statePlayL;
    public bool statePlayR;
            ...
            switch (_Gestures.TargetHand)
            {
                case CGEnums.HandFlag.Either:
                    ...
                    if (CustomGestureDefiner.IsCurrentGestureTriiggered(_Gestures.GestureName, CGEnums.HandFlag.Left) && CheckHandValid(CGEnums.HandFlag.Left))
                    {
                        ...
                        {
                            startRecord[0].text = "Recording state";
                            //st = true;
                        }
                        else if (_Gestures.GestureName == "Five" && startRecord[0].text == "Recording state")
                        {
                            startRecord[0].text = "Recording......";
                            stateRecL = true;
                        }
                        else if (_Gestures.GestureName == "OK")
                        {
                            startRecord[0].text = "No state";
                            stateRecL = false;
                        }

                        if (_Gestures.GestureName == "Like")
                        {
                            startRecord[0].text = "Playing state";
                            //st = true;
                        }
                        else if (_Gestures.GestureName == "Point" && startRecord[0].text == "Playing state")
                        {
                            startRecord[0].text = "Playing......";
                            statePlayL = true;
                        }
                        else if (_Gestures.GestureName == "OK")
                        {
                    ...
                    if (CustomGestureDefiner.IsCurrentGestureTriiggered(_Gestures.GestureName, CGEnums.HandFlag.Right) && CheckHandValid(CGEnums.HandFlag.Right))
                    {
                        ...
                        if (_Gestures.GestureName == "Fist")
                        {
                            startRecord[1].text = "Recording state";
                            //st = true;
                        }
                        else if (_Gestures.GestureName == "Five" && startRecord[1].text == "Recording state")
                        {
                            startRecord[1].text = "Recording......";
                            stateRecR = true;
                        }
                        else if (_Gestures.GestureName == "OK")
                        {
                            startRecord[1].text = "No state";
                            stateRecR = false;
                        }

                        if (_Gestures.GestureName == "Like")
                        {
                            startRecord[1].text = "Playing state";
                            //st = true;
                        }
                        else if (_Gestures.GestureName == "Point" && startRecord[0].text == "Playing state")
                        {
                            startRecord[1].text = "Playing......";
                            statePlayR = true;
                        }
                        else if (_Gestures.GestureName == "OK")
                        {
